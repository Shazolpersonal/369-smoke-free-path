...
import Animated, { FadeIn, FadeOut, useAnimatedStyle, useSharedValue, withSpring, withRepeat, withSequence, withTiming, Easing } from 'react-native-reanimated';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (trueStreak > 1) {
            fireScale.value = withRepeat(
                withSequence(
                    withTiming(1.2, { duration: reducedMotion ? 0 : 1000, easing: Easing.inOut(Easing.ease) }),
                    withTiming(1, { duration: reducedMotion ? 0 : 1000, easing: Easing.inOut(Easing.ease) })
                ),
                -1, true
            );
        }
    ...
    const showRestBanner = isRestPeriod && !isSlotActive('morning') && !isSlotActive('noon') && !isSlotActive('night');
    ...
    return (
        ...
        >
                        ...
                        {isJourneyComplete(totalElapsedDays) ? (
                            ...
                            <>
                                ...
                                <View style={{ marginTop: 28, alignItems: 'center' }}>
                                    ...
                                    {currentCycle > 1 && (
                                        <Text style={[{ color: '#D4A847', marginTop: 12, fontSize: 13 }, { fontFamily: f('semibold') }]}>
                                            {t('home.progress.cycle').replace('{cycle}', currentCycle.toString())}
                                        </Text>
                                    )}
                                </View>
                            </>
            ...
            {/* ─── Bottom Sheet Modal ─── */}
            <BottomSheet key={language} visible={modalVisible} onClose={() => setModalVisible(false)}>
