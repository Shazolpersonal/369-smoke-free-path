  ...
  return (
    ...
    >
      ...
      <View style={styles.statCardInner}>
        ...
        <Text
          style={[styles.statValue, { fontFamily: fontFamily("bold"), color }]}
        >
...
export default function HistoryScreen() {
  ...
  const [viewDate, setViewDate] = useState(
    () => new Date(getEffectiveTodayDate()),
  );
  ...
  const calendarDays = useMemo(() => {
    ...
    const m = viewMonth + 1;
    const formattedMonth = `${m < 10 ? '0' : ''}${m}`;
    for (let d = 1; d <= totalDays; d++) {
      // ⚡ Bolt: Direct conditional interpolation avoids String().padStart() allocations
  ...
  const handleNextMonth = () => {
    ...
    if (
      nextYear < todayYear ||
      (nextYear === todayYear && nextMonthIndex <= todayMonth)
    ) {
      setDirection("right");
  ...
  const monthStats = useMemo(() => {
    ...
    for (const cell of calendarDays) {
      ...
      // ⚡ Bolt: Use lexicographical string comparisons on ISO-formatted dates
      // instead of instantiating new Date objects inside the loop
            ...
            <View style={styles.legendRow}>
              {legendItems.map(({ color, labelKey }) => (
                <View key={labelKey} style={styles.legendItem}>
                  ...
                  />
                  <Text
                    style={[styles.legendText, { fontFamily: f("medium") }]}
                  >
                    {t(labelKey)}
                  </Text>
                </View>
      ...
      >
        <View style={styles.modalContent}>
          <Text style={[styles.modalTitle, { fontFamily: f("bold") }]}>
            {selectedDay
              ? formatDateKeyHumanReadable(selectedDay, language)
              : ""}
          </Text>
          ...
          {selectedProgress ? (
            <View style={styles.modalSlots}>
              {[
                {
                  key: "morning",
                  label: t("history.morningAffirmation"),
                  done: selectedProgress.morning,
                  emoji: "🌅",
                },
                {
                  key: "noon",
                  label: t("history.afternoonAffirmation"),
                  done: selectedProgress.noon,
                  emoji: "☀️",
                },
                {
                  key: "night",
                  label: t("history.eveningAffirmation"),
