...
export const calculateEffectiveElapsedDays = (startDateKey: string): number => {
  ...
  // Parse start date
  // ⚡ Bolt: Fast parsing of YYYY-MM-DD avoiding array allocations
  const startYear = parseInt(startDateKey.substring(0, 4), 10);
  const startMonth = parseInt(startDateKey.substring(5, 7), 10);
  const startDay = parseInt(startDateKey.substring(8, 10), 10);
  
  // Parse current effective date
  const currentYear = parseInt(currentEffectiveDateKey.substring(0, 4), 10);
  const currentMonth = parseInt(currentEffectiveDateKey.substring(5, 7), 10);
  const currentDay = parseInt(currentEffectiveDateKey.substring(8, 10), 10);
