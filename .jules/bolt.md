...
## 2025-06-25 - React Context Value Derivations Re-evaluating on Local State Change
**Learning:** Components consuming context values (like `dailyProgress` from `useProgress`) that derive complex or expensive data structures inside their render bodies (e.g., calling `getAchievements()`) will re-evaluate those complex derivations whenever *any* unrelated local state changes (e.g., toggling a modal, changing an active animation ID).
**Action:** Always wrap expensive synchronous derivations inside consuming components with `useMemo`, ensuring the dependencies strictly tie to the context values and ignore local, high-frequency state changes.
