...
## 2025-06-25 - React re-rendering unmemoized functions
**Learning:** The getAchievements function was called on every render in the Achievements component, even when the underlying data hadn't changed, causing unnecessary recalculations when local component state (like active badge) changed.
**Action:** Memoize expensive calculations that derive from props or contexts using useMemo, especially when the component contains local state that triggers frequent re-renders.
