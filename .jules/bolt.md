...
## 2025-06-26 - Missing memoization for expensive component properties
**Learning:** Calling complex derivational functions (like `getAchievements` which counts iterations across unbounded user state `dailyProgress`) directly within the render loop without memoization causes unnecessary recalculations and UI stutter during unrelated local state updates.
**Action:** Wrap computationally expensive derived values using `useMemo` when passing them into the UI or components to prevent redundant recalculations on local state changes.
