...
## 2025-01-16 - Parallelize Notification Scheduling
**Learning:** Sequential `await` in loops for independent I/O tasks (like `Notifications.scheduleNotificationAsync`) accumulates latency.
**Action:** Always use `Promise.all()` to parallelize independent asynchronous operations to improve execution time, especially when iterating over small, fixed-size arrays.
