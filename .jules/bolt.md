...
## 2025-07-08 - Unnecessary Object Dependency in useMemo
**Learning:** Including frequently updating, unrelated state variables (like an entire `dailyProgress` dictionary) in the dependency array of a `useMemo` hook meant to derive simple static values (like `currentCycle` from `totalElapsedDays`) defeats the purpose of memoization and triggers severe performance degradation due to pointless recalculations.
**Action:** Always strictly limit the dependency array of `useMemo` and `useCallback` to variables that are actively used in the calculation.
