...
## 2025-07-15 - Sequential I/O Blocking in Notification Scheduling
**Learning:** Sequential `await` in loops for independent I/O tasks (such as scheduling multiple local notifications) introduces unnecessary cumulative latency that delays subsequent operations.
**Action:** Avoid sequential `await` in loops for independent tasks; parallelize using `Promise.all()` to eliminate cumulative latency and improve execution time significantly.
