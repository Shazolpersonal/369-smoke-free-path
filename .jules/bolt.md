...
## 2025-07-28 - Independent Async I/O in `utils/scheduler.ts` and `utils/notifications.ts`
**Learning:** Sequential `await` calls in a `for...of` loop or successive statements for scheduling push notifications cause cumulative latency by blocking the main thread pointlessly, given that the underlying `scheduleNotificationAsync` calls are functionally independent.
**Action:** Always map the independent operations to an array of promises and await them concurrently using `Promise.all()`. This eliminates cumulative blocking time and yields dramatic (~66%) latency reductions.
