...
## 2025-02-12 - Prevent Re-calculation of Achievements on Local State Changes
**Learning:** Recomputing complex statistics or rendering components that rely on derived values (like calling `getAchievements` that iterates over `dailyProgress`) can happen unnecessarily on unrelated local state changes (e.g., triggering confetti by setting `activeBadgeId`). This can cause performance hiccups during animations or interactions.
**Action:** Always wrap complex derivations (like `getAchievements`) in `useMemo` with their respective dependencies (e.g., `dailyProgress`, `trueStreak`, `totalElapsedDays`) to prevent unnecessary recalculations on local state updates.
