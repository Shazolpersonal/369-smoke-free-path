...
## 2025-06-25 - Extraneous computations due to local state
**Learning:** Calculating derived state (`getAchievements`) via expensive operations in components with rapid local state updates (like triggering a confetti animation on press) causes unnecessary recalcluations leading to performance degradation.
**Action:** Wrap these expensive calculations in a `useMemo` block with correct dependencies so they only re-run when the source of truth changes (e.g. `dailyProgress`).
