...
## 2025-06-26 - Missing Memoization on Complex Derived State
**Learning:** Calculating derived state that relies on expensive loops (like `getAchievements` generating the badges array based on historical progress) on every re-render causes unnecessary computational overhead, especially when unrelated local state changes (e.g. `activeBadgeId` updating for a confetti animation).
**Action:** When deriving complex statistics or rendering components that rely on the `useProgress()` hook's values (like `dailyProgress`, `trueStreak`, `totalElapsedDays`), ensure the resulting calculations are wrapped in `useMemo` to prevent unnecessary recalculations.
