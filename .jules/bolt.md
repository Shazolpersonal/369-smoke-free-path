...
## 2026-07-01 - Unnecessary Context Re-renders Due to Extraneous Dependencies\n**Learning:** Including primitive values or frequently updating state variables (like `dailyProgress`) in `useMemo` dependency arrays for derived functions that do not actually use them forces unnecessary recalculations and context re-renders, impacting performance. The `currentDayInCycle` and `currentCycle` derivations unnecessarily included `dailyProgress` causing re-evaluations whenever any progress was logged.\n**Action:** Always strictly audit `useMemo` and `useCallback` dependency arrays to ensure they only contain variables actually utilized within the function body, stripping out placeholder or loosely related state variables.

## 2026-07-01 - Memoizing Expensive Computations in Components
**Learning:** Components that rely on derived data from context (such as `getAchievements` taking `dailyProgress` and `trueStreak`) will recalculate those values on every local state change (e.g., clicking a badge setting an active ID). This causes severe O(N) array allocation overhead during simple UI interactions.
**Action:** Wrap expensive derivation functions in `useMemo` within the component itself when the result depends strictly on context values, preventing full recalculation when unrelated local state (like active/hover states) changes.
