...
## 2026-05-22 - Notification Scheduling Bottleneck
**Learning:** Using sequential `await` inside loops for independent asynchronous tasks (like `Notifications.scheduleNotificationAsync`) creates a waterfall effect that unnecessarily increases total execution time. Since these scheduling operations don't depend on each other, they should be run concurrently.
**Action:** Always map independent async calls to an array of promises and await them concurrently using `Promise.all()`.
