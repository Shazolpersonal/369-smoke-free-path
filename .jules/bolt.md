...
## 2026-07-04 - Missing Memoization in Achievements Component
**Learning:** The `getAchievements` utility was being called directly inside the render body of `components/Achievements.tsx`. While the utility itself is relatively fast (O(N)), executing it on every re-render (such as when toggling local state for UI animations) causes unnecessary array allocations and recalculations.
**Action:** When a component relies on derived state (like a list of badges computed from a daily progress dictionary) and has its own frequent local state updates (like `activeBadgeId` for a confetti animation), always wrap the derived state calculation in `useMemo`.
