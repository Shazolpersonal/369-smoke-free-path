...
## 2025-06-25 - Sequential Asynchronous I/O Overhead
**Learning:** Using sequential `await` statements in loops (e.g., `for...of` loops scheduling multiple notifications) blocks thread execution unnecessarily for independent I/O tasks. This leads to cumulative latency, which degrades performance, especially on slower mobile devices where bridging over to native modules adds overhead.
**Action:** When executing independent asynchronous tasks like scheduling multiple notifications, always parallelize them using `Promise.all()` to eliminate cumulative latency and improve overall execution time.
