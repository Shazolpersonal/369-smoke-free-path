...
## 2026-06-22 - Unnecessary Recalculations of Complex Statistics on Render
**Learning:** Re-evaluating complex derivation functions (like `getAchievements`) inside component renders causes wasteful CPU usage, particularly when unrelated local state changes (e.g. an interactive `activeBadgeId` that triggers confetti animations) cause frequent re-renders.
**Action:** Always wrap derived statistics calculations in a `useMemo` hook, especially when they depend on contexts that update less frequently (like `dailyProgress`, `trueStreak`, and `totalElapsedDays`).
