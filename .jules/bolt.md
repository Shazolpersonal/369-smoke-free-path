...
## 2025-07-28 - Optimizing Context Hooks and Derived State with `useMemo`
**Learning:** Returning values in Context Providers that are derived using `useMemo` with unnecessary dependencies (like including the large `dailyProgress` object when only `totalElapsedDays` is needed) causes severe performance degradation, forcing all components consuming the context to re-render uselessly on every minor state update. Likewise, expensive O(N) functional loops inside React components (like `getAchievements` traversing the entire progress dictionary) must be memoized based on exactly what they consume to avoid recalculations triggered by unrelated local state updates (like active UI states/animations).
**Action:** Always strictly limit `useMemo` dependency arrays to the exact variables required for the calculation. When deriving complex statistics using functional utilities inside components, always wrap the resulting output in `useMemo` to ensure the calculation only runs when the underlying data changes, not on every render.
