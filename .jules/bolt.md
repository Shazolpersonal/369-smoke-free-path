...
## 2025-06-25 - React Component Recalculation Overhead
**Learning:** Complex statistical functions (like `getAchievements`) that iterate over large objects (like `dailyProgress` tracking months of data) will execute on every component re-render if not memoized, causing unnecessary O(N) processing overhead, especially when unrelated local state (like a UI active state or selected badge) changes.
**Action:** Always wrap heavy data transformation functions (especially those containing O(N) loops) inside a `useMemo` hook with strict dependencies when placed inside a React component to prevent recalculation during unrelated renders.
