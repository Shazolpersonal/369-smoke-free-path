...
## 2025-06-25 - React Component Array Allocation Overhead
**Learning:** Calling functions that return new arrays (like `getAchievements` based on `dailyProgress`) directly in a React component's render body causes the application to recalculate and allocate a new array reference on every single re-render, even for unrelated local state changes (e.g., toggling a badge's active state for an animation). This creates measurable performance degradation as the data structures grow.
**Action:** Always wrap expensive or array/object-returning derivation functions (like calculating achievements or complex statistics from context values) inside `useMemo`, strictly defining the required dependencies.
