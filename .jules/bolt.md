...
## 2026-08-28 - Unnecessary Recalculation of Derived State in Render Loops
**Learning:** Functions that derive complex statistics or aggregate objects (like `getAchievements`) can cause significant CPU overhead and memory churn if called on every render. This is especially problematic when the component has local state that updates frequently (e.g., active/focus states, modal visibility, or animation values).
**Action:** Always wrap expensive or complex derived calculations in a `useMemo` hook, strictly defining dependencies, to prevent redundant execution during unrelated state updates.
