...
## 2026-05-31 - Parallelize I/O bounds
**Learning:** Sequential `await` loops over non-dependent I/O tasks like API calls or notification schedules create an artificial bottleneck from cumulative latency, whereas concurrent execution mitigates this.
**Action:** Always map the independent promises into an array and resolve them concurrently with `Promise.all()` instead of iterating sequentially with `await`.
