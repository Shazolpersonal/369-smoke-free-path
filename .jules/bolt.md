...
## 2025-10-27 - Cumulative Latency in Sequential Notification Scheduling
**Learning:** Using sequential `await Notifications.scheduleNotificationAsync(...)` calls inside a loop or consecutive blocks creates unnecessary cumulative latency, especially noticeable in React Native where native bridge crossings take time.
**Action:** Always batch independent asynchronous I/O tasks like scheduling multiple notifications using `Promise.all(...)` to run them concurrently, eliminating sequential delay.
