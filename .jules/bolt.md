...
## 2025-02-14 - Parallelize independent I/O loop tasks
**Learning:** In the `utils/scheduler.ts` and `utils/notifications.ts` files, I/O tasks like `Notifications.scheduleNotificationAsync()` inside `for` loops or executed sequentially introduce unnecessary cumulative latency. The environment runs JavaScript, and these native module calls are highly parallelizable via `Promise.all()`.
**Action:** When scheduling multiple independent notifications or performing batch operations via asynchronous functions, always use `Promise.all(tasks)` instead of a sequential `for...of` loop with `await` to eliminate cumulative wait time.
