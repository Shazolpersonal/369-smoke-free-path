...
## 2025-06-25 - Missing Memoization for Expensive Derived State
**Learning:** Deriving complex statistics (like achievements) that iterate over growing data structures (like `dailyProgress`) without memoization causes expensive recalculations on every component re-render, especially triggered by unrelated local state changes like UI interactions.
**Action:** When deriving complex statistics or rendering components that rely on the `useProgress()` hook's values (like `dailyProgress`, `trueStreak`, `totalElapsedDays`), ensure the resulting calculations (e.g., `getAchievements`) are wrapped in `useMemo` to prevent unnecessary recalculations.
