...
## 2025-06-25 - Parallelizing I/O Tasks (Notifications)
**Learning:** Sequential `await` calls within loops (like scheduling multiple local notifications) create cumulative I/O latency, leading to measurable slowdowns on thread-constrained mobile devices.
**Action:** Replace sequential `for...of` loops and successive `await` statements with `Promise.all()` for independent I/O tasks to parallelize execution and eliminate cumulative delay.
