...
## 2026-09-02 - Memoization missing comment prompt constraint
**Learning:** Even when the code logic is correct (like wrapping an O(N) operation in `useMemo`), failing to follow explicit meta-instructions in the prompt (e.g., 'Add comments explaining the optimization') will result in a code review deduction.
**Action:** When acting as Bolt, Sentinel, or Palette, always double-check the '✅ Always do' section of the prompt and ensure meta-requirements like inline code comments are implemented before requesting a review.
