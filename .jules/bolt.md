...
## 2025-02-12 - Missing dependencies constraint validation
**Learning:** Jest tests fail locally due to missing `node_modules` (specifically `jest-expo` preset), and `package.json` does not expose lint or format scripts. This prevents standard CLI verification steps.
**Action:** When standard test/lint runners are unavailable or broken, use inline Node scripts (e.g. `node -e`) to search file contents with string matching or regex to verify that patches are correctly applied.
