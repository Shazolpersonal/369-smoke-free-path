...
**Learning:** Functions that derive complex state or iterate over collections (e.g., `getAchievements` iterating over `dailyProgress`) can cause significant CPU overhead and dropped frames if called on every render.
**Action:** Always wrap expensive derived state calculations in `useMemo` when rendering components that rely on context or frequently changing local state, ensuring recalculation only happens when explicit dependencies change.
