...
## 2025-06-25 - Redundant Function Re-evaluation on State Change
**Learning:** Components containing local state that updates frequently (like tapping an element changing `activeBadgeId`) will trigger re-renders, causing expensive functions (like deriving complex arrays based on context values) to run unnecessarily on every render cycle.
**Action:** Use React's `useMemo` hook to wrap expensive or complex derivations of external data (like context values or props) to prevent recalculations when unrelated local component state changes.
