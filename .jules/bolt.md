...
## 2025-07-15 - Redundant Sequential Await for Independent Notifications
**Learning:** Using sequential `await` in a `for...of` loop or individual await calls to schedule multiple independent notifications creates cumulative latency. Since the notifications do not depend on each other, waiting for one to finish before starting the next is an anti-pattern that slows down background processing and app initialization.
**Action:** Always parallelize independent asynchronous I/O tasks like scheduling notifications using `Promise.all()`. This executes the tasks concurrently, significantly reducing execution time.
