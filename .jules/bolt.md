...
## 2025-06-25 - Missing Memoization in Component Stats
**Learning:** When deriving complex statistics or rendering components that rely on the `useProgress()` hook's values (like `dailyProgress`, `trueStreak`, `totalElapsedDays`), failing to wrap the resulting calculations (e.g., `getAchievements`) in `useMemo` causes unnecessary O(N) recalculations on unrelated local state changes (e.g. toggling active badge confetti).
**Action:** Ensure expensive derived calculations from context are wrapped in `useMemo` to prevent performance bottlenecks during local re-renders.
