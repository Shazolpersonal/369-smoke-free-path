...
## 2025-07-28 - Independent Async Operation Parallelization
**Learning:** Sequential `await` in loops (like scheduling multiple notifications) causes cumulative latency for independent I/O tasks.
**Action:** Use `Promise.all()` to execute independent async operations concurrently whenever possible, significantly reducing overall execution time.
