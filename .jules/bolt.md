...
## 2025-06-30 - Memoizing Render-Blocking Calculations (React context derived values)
**Learning:** Functions like `getAchievements` that process object keys (e.g. `dailyProgress`) can run multiple times per render if they depend on context variables but lack memoization. This results in heavy, unneeded O(N) operations whenever unrelated local states (like an `activeBadgeId` flag) are updated.
**Action:** When a UI component consumes complex objects via context hooks and passes them into utility functions to derive UI state, aggressively wrap those utility function calls in `useMemo` with only their strict dependencies.
