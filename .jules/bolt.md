...
## 2023-10-24 - Memoizing Expensive Derived State
**Learning:** Functions that process large sets of data (e.g., iterating through a large `dailyProgress` dictionary in `getAchievements`) should not be executed on every render, especially when the component contains local state that updates frequently (e.g., `activeBadgeId` toggling animations). Not wrapping such calculations in `useMemo` causes an O(N) operation to run unnecessarily on minor interactions, leading to jank and increased battery usage on mobile.
**Action:** Always wrap derived datasets or complex statistics that depend on `useProgress()` values (like `dailyProgress`) in `useMemo`, with strict dependency arrays to ensure recalculation only happens when the underlying data actually changes.
