...
## 2025-10-25 - Notification Scheduling Latency
**Learning:** Using sequential `await` loops for independent asynchronous I/O operations (like scheduling multiple daily push notifications via `expo-notifications`) introduces cumulative latency (e.g., 150ms vs 50ms) that blocks the thread unnecessarily during startup or background tasks.
**Action:** When scheduling or interacting with multiple independent native modules or I/O bounds tasks in a fixed set (like `TIME_SLOTS`), always map them to an array of promises and use `Promise.all()` to execute them concurrently, effectively eliminating the cumulative latency penalty.
