...
## 2025-07-02 - Unnecessary useMemo Dependencies in React Context
**Learning:** Including unrelated, frequently updating state variables (like `dailyProgress`) in a `useMemo` dependency array just to "force" re-renders is an anti-pattern. If the derived value is primitive, it won't change, and if the context already includes the state variable in its provider value, consumers will re-render anyway. This causes unnecessary recalculations and performance degradation.
**Action:** Strictly limit `useMemo` dependency arrays to variables actively used in the calculation.
