...
## 2025-02-12 - Parallelizing Independent I/O Tasks
**Learning:** Sequential `await` statements for independent I/O tasks (such as scheduling multiple notifications in a loop) introduce cumulative latency, slowing down execution significantly.
**Action:** Always parallelize independent asynchronous operations using `Promise.all()` to eliminate unnecessary delays and improve performance.
