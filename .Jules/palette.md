...
## 2025-05-18 - Toast Notification Accessibility
**Learning:** React Native alerts and custom toast notification components using `Animated.View` do not naturally announce themselves to screen readers when they appear on screen.
**Action:** Always add `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and `accessibilityLabel` mapping to the notification text to the root container of custom toast notifications to ensure screen readers announce them properly.
