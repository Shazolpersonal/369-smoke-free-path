...
## 2024-06-24 - Accessibility Role for Alert/Toasts
**Learning:** Found an opportunity to improve the UX for screen readers when using custom animated Toast/Alert notifications by adding `accessible={true}`, `accessibilityRole="alert"`, and `accessibilityLiveRegion="polite"` to the root `Animated.View`. This allows screen readers to naturally announce the dynamically appearing toast notification message.
**Action:** For custom animated Toast/Alert components, always add `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and a comprehensive `accessibilityLabel` containing the message to the root container to ensure they are properly announced.
