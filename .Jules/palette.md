...
## 2024-05-20 - Adding Accessibility to Toast Notifications
**Learning:** Toast or alert notification components built with custom views (like `Animated.View`) are completely invisible to screen readers unless explicitly marked. They appear and disappear too quickly for manual navigation.
**Action:** Always ensure custom toast or alert notification components are properly announced by adding `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and a comprehensive `accessibilityLabel` containing the notification message to their root container.
