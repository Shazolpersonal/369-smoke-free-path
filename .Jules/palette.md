...
## 2025-05-19 - Making Toast Notifications Screen Reader Accessible
**Learning:** Found that custom in-app notifications (like `Toast.tsx`) were visually informative but lacked ARIA roles and labels, rendering them invisible to screen readers since they appear dynamically outside of standard user focus flow.
**Action:** Always wrap dynamic visual notification components with `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and an `accessibilityLabel` containing the notification message to ensure screen readers announce the dynamic update.
