...
## 2024-05-18 - Making Custom SVG Progress Rings Accessible
**Learning:** Pure UI components built with React Native SVG primitives (like `<Circle>` and `<Svg>`) are entirely invisible to screen readers unless the parent container is explicitly marked with `accessible={true}` and `accessibilityRole="progressbar"`. Additionally, omitting `accessibilityValue` and `accessibilityLabel` means users with visual impairments have no way to understand what the rings represent or what the current progress is.
**Action:** When creating custom progress indicators (like `JourneyProgressRing` and `RepetitionCounter`), always apply `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a descriptive `accessibilityLabel` to the root container.
