...
## 2025-06-13 - SVG Custom Components as Progress Bars
**Learning:** Found that custom SVG components in React Native (such as `JourneyProgressRing`) which visually act as progress bars are completely opaque to screen readers by default.
**Action:** When implementing custom SVG-based progress rings or counters, ensure the outer container sets `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a descriptive `accessibilityLabel` to communicate the progress state effectively.
