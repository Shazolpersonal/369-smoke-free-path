...
## 2024-05-18 - Toast Component Accessibility
**Learning:** React Native's `Animated.View` components acting as notifications or transient alerts often lack implicit accessibility roles, causing them to be ignored by screen readers when they appear.
**Action:** Always add `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and an explicitly computed `accessibilityLabel` containing the notification's message to the root container of custom alert/toast UI components.
