...
## 2026-06-22 - Toast Accessibility Fix
**Learning:** The custom Toast component relied strictly on visual notification indicators and completely missed screen reader announcements. Adding standard React Native `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` properties alongside `accessibilityLabel` properly brings context to screen readers during dynamic alert pop-ups.
**Action:** For all dynamically appearing notification UI components (like Toasts, Snackbars, or alerts), always assign `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` in their root view.
