...
## 2024-06-07 - Screen Reader Announcements for Toast Notifications
**Learning:** Found an opportunity to improve the UX for visually impaired users. In React Native, components that serve as transient notifications (like toast or alert dialogs) are not automatically announced by screen readers when they appear on screen. They must be explicitly configured as alerts.
**Action:** For React Native toast or alert notification components (like `Toast.tsx`), ensure they are properly announced to screen readers by adding `accessible={true}`, `accessibilityRole="alert"`, `accessibilityLiveRegion="polite"`, and a comprehensive `accessibilityLabel` containing the notification message to the root container.
