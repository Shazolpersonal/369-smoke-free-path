...
## 2026-06-09 - Adding proper progressbar accessibility to custom SVG rings
**Learning:** When implementing custom SVG-based progress rings or counters in React Native, ensure the outer container sets `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue` (with min, max, and now), and a consolidated `accessibilityLabel` to provide proper context to screen readers, as SVG inner nodes are often skipped.
**Action:** Always add consolidated accessibility props to the root container of custom SVG components used for tracking progress.
